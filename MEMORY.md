...
## 2026-06-08 - UI Consistency Fixes
- **Inventory Expanded Row UI**: Fixed the background color of the expanded row container in `Inventory.jsx` to perfectly match the table content of the `Approvals.jsx` page (using `bg-gray-50/50 dark:bg-page` and `bg-white dark:bg-card`). This eliminates the issue where the VM content appeared "too white" in dark mode.
- **Inventory Row Highlight Bug Fix**: Fixed an issue where the main VM row in `Inventory.jsx` became bright white in dark mode when expanded. The root cause was `dark:bg-surface/30` being invalid CSS (opacity modifiers fail on hex variables like `--bg-surface`), which caused it to fall back to the light-theme `bg-gray-50/80`. To strictly match the design in `Approvals.jsx` (which does not highlight the main row when expanded), the `isExpanded` background color override was removed entirely. The main row now retains its standard table background and normal hover effects, just like the approval request menu.
- **Inventory Expanded Description UI**: Standardized the "Description / Notes" container within the expanded VM row in `Inventory.jsx` to perfectly match the "Description / Purpose" container in `Approvals.jsx`. Replaced the previous styling (`bg-white dark:bg-card rounded-card`) with the identical soft-layered design (`bg-gray-50 dark:bg-page rounded-lg`) and matched all header typography and colors.
- **Standardizing Forms**: Applied the "Create Role" layout standard to all remaining management modals (`NetworkForm.jsx`, `DatastoreForm.jsx`, `EnvironmentForm.jsx`, and `TierForm.jsx`). 
    - Removed shadow-2xl / rounded-2xl styles.
    - Updated inputs to simple `border border-slate-300`, `rounded-md`, `bg-white/bg-page`.
    - Added the simple `p-4 border-b/border-t` footer and header.
    - Added scrollable `overflow-y-auto` behavior to fit nicely within standard screen sizes.
- **Provider Form Section Styling**: Re-designed the sections inside the "Add Provider" / "Edit Provider" modal (`ProviderForm.jsx`) to adopt the enclosed box layout seen in the "Create Role" modal (`RoleForm.jsx`). Wrapped each section (Basic Information, Credentials, Configuration) in a `bg-slate-50 dark:bg-slate-800/50` container with subtle borders and rounded edges. Also removed the dark/transparent footer styling (`bg-transparent dark:bg-transparent/50` with negative margins) to make the footer blend seamlessly with the modal's white/card background, matching the exact footer design of the "Create Role" modal.
- **Provider Form Scrolling Layout**: Fixed the layout height of the `ProviderForm` modal so it doesn't grow indefinitely off-screen. Set a maximum height (`max-h-[90vh]`) to the modal wrapper and restructured the internal `form` using flexbox (`overflow-hidden min-h-0`). The main fields are now wrapped in an auto-scrolling container (`overflow-y-auto custom-scrollbar`), while the header and footer remain firmly pinned to the top and bottom.
- **Inventory Modals**: Standardized the design for `Delete VM`, `Renew VM Request`, and `Edit VM Resources` modals in `Inventory.jsx` to perfectly match the unified "Create Role" design. Applied the same gray-50 content boxes, `max-h-[90vh]` limits, `overflow-y-auto` scroll areas, header `border-b` styling with close icons, and solid-background footers with `border-t`. Removed custom background transparencies to fix dark mode bleeding issues.
- **Approvals Modals**: Standardized the design for `Approve`, `Reject`, and `Revert` action modal in `Approvals.jsx` to match the "Create Role" layout. Enclosed the reason text area in a `bg-slate-50 dark:bg-slate-800/50` box, added a proper top border and X button to the header, added a solid footer, and constrained the height with `max-h-[90vh]` and `overflow-y-auto`.
- **Request VM Modal**: Standardized the confirmation modal when submitting a new VM provision request (`VmRequest.jsx`). Applied the `max-h-[90vh] flex flex-col` pattern, solid header and footer borders, the standard "X" close button, and enclosed the main prompt inside the `bg-slate-50` box container.
