...
class VmStateChanged implements ShouldBroadcast
{
    ...
    /**
     * Light real-time push runs on its own `system` queue with a dedicated worker, so it is never
     * stuck behind heavy Terraform provisioning jobs on the `default` queue (keeps the UI live
     * during batch provisioning). See scripts/backend.sh worker_sys.
     */
