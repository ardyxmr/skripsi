...
class InventoryController extends Controller
{
    ...
    // Reveal the per-VM login credentials. RBAC-scoped (owner/manager/admin via authorizeView) and
    // AUDITED, because it exposes a secret — the password is never in the inventory list, only here.
    // The model `encrypted` cast decrypts the password on read; null for VMs provisioned pre-feature.
    public function credentials(Request $request, Inventory $inventory, AuditService $audit): JsonResponse
    {
        $this->authorizeView($request->user(), $inventory);
        $audit->log($request->user(), 'VIEW_CREDENTIALS', "Viewed login credentials for {$inventory->vm_name}");

        return response()->json([
            'username' => $inventory->login_username,
            'password' => $inventory->login_password,
        ]);
    }
    ...
    // Run a SELECTED catalog hardening version against the VM (config-only, ADR-14). Routes through
    // LifecycleService → approval for users, immediate for Mgr/Admin. The chosen version_id is carried
    // in the approval payload, so an approval honored later runs the version selected (not "latest").
    public function harden(Request $request, Inventory $inventory, LifecycleService $lifecycle): JsonResponse
    {
        $this->authorizeActionable($request->user(), $inventory);
        abort_unless($inventory->status === 'Active', 422, 'Hardening can only run on an Active VM.');
        abort_if($inventory->hardening_status === 'Running', 422, 'Hardening is already running for this VM.');
        ...
        // The version must belong to THIS VM's catalog and still be active.
        $version = CatalogHardeningVersion::where('catalog_id', $inventory->catalog_id)
            ->where('is_active', true)->find($data['version_id']);
        abort_unless($version, 422, 'That hardening version is not available for this VM.');

        // Re-running the SAME version already applied is allowed only with an explicit force (drift
        // remediation); the UI shows a confirm. A different version proceeds normally.
        abort_if(
            ! ($data['force'] ?? false)
                && $inventory->hardening_status === 'Success'
                && (int) $inventory->hardened_version_id === $version->id,
            422,
            'This version is already applied. Confirm a force re-harden to run it again.'
        );

        return response()->json($lifecycle->request(
            $inventory, 'HARDEN', ['version_id' => $version->id],
            $data['description'] ?? "Hardening requested ({$version->name} {$version->version})", $request->user(),
        ), 202);
    }
    ...
    private function transform(Inventory $i): array
    {
        return [
            ...
            'created_at' => $i->created_at,            // row creation timestamp → "Created Date"
            ...
            'last_hardened_at' => $i->last_hardened_at,
            // Versioning: drives the Inventory "Hardening" action (shown when the catalog has any active
            // version) + the modal's "already applied" detection (applied_version_id vs the selected row).
            'catalog_has_hardening' => (bool) $i->catalog?->hasHardening(),
            'applied_version_id' => $i->hardened_version_id,
            'applied_version' => $i->hardenedVersion ? [
                'name' => $i->hardenedVersion->name,
                'version' => $i->hardenedVersion->version,
            ] : null,
