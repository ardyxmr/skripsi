...
class EditResourcesVmJob implements ShouldQueue
{
    ...
    public function handle(WorkspaceService $workspaces, TerraformRunner $terraform, AuditService $audit): void
    {
        ...
        foreach ($this->disks as $d) {
            ...
            if ($existing >= $limit) {
                $audit->log($actor, 'EDIT_RESOURCES', "Add-disk skipped for {$vm->vm_name}: data-disk cap reached ({$existing}/{$limit})",
                    null, $vm->auditMeta(['result' => 'disk_skipped', 'size_gb' => $size, 'used' => $existing, 'limit' => $limit]));
                break;
            }
            $slot = 'scsi'.$nextIndex;
        ...
        if (! $result['ok']) {
            ...
            $audit->log($actor, 'EDIT_RESOURCES', "Edit Resources FAILED {$vm->vm_name} (step {$result['step']})",
                null, $vm->auditMeta(['result' => 'failed', 'step' => $result['step']]));
        ...
        $audit->log($actor, 'EDIT_RESOURCES', "Edited {$vm->vm_name}: ".implode(', ', $changes),
            null, $vm->auditMeta(['result' => 'success', 'changes' => $changes]));
