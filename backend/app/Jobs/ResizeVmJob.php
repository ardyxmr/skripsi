...
class ResizeVmJob implements ShouldQueue
{
    ...
    public function __construct(public int $inventoryId, public ?int $cpu = null, public ?int $ramMb = null, public ?int $actorId = null) {}
    ...
    public function handle(WorkspaceService $workspaces, TerraformRunner $terraform, AuditService $audit): void
    {
        ...
        if (! $result['ok']) {
            ...
            $audit->log($actor, 'RESIZE_VM', "Resize FAILED {$vm->vm_name} (step {$result['step']})",
        ...
        $audit->log($actor, 'RESIZE_VM', "Resized {$vm->vm_name} → {$onlineVcpu} vCPU / {$resolved['memory']} MB",
            null, $vm->auditMeta(['result' => 'success', 'new_vcpu' => $onlineVcpu, 'new_ram_mb' => $resolved['memory']]));
