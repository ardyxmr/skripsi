...
class LifecycleService
{
    ...
    private function applyChange(Inventory $vm, string $type, array $payload, ?User $actor): void
    {
        switch ($type) {
            case 'RENEWAL':
                ...
                if ($newExpiry) {
                    $vm->update(['expiry_date' => $newExpiry, 'status' => $vm->status === 'Expired' ? 'Active' : $vm->status]);
                }
                break;
        ...
        $this->audit->log($actor, "APPLY_{$type}", "Applied {$type} to {$vm->vm_name}");
    }
    ...
    private function cappedRenewal(Inventory $vm, string $period): ?Carbon
    {
        ...
        $newExpiry = $cap ? Carbon::minimum($requested, $cap) : $requested;
