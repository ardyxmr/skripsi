...
class WorkspaceService
{
    ...
    {
        // Defense-in-depth: vm_name is charset-validated at the request layer, but this value is
        // interpolated into a filesystem path below — never allow anything that could traverse out
        // of the workspace root (/, .., .) or otherwise escape the expected DNS-label charset.
        if (! preg_match('/^[A-Za-z0-9-]{1,63}$/', $vmName)) {
            throw new \InvalidArgumentException("Unsafe vm_name for workspace path: {$vmName}");
        }
