...
class ResourceResolutionService
{
    ...
    public function resolve(ProvisionRequest $pr, string $vmName, ?int $bootDiskGb = null): array
    {
        ...
        return [
            'vm_name' => $vmName,
            // Per-VM cloud-init login. Alphanumeric only (no symbols) so the value is safe inside the
            // double-quoted HCL tfvars string (no `${}` interpolation / quote-escaping surprises) and
            // in cloud-init. Stored encrypted on the inventory row by ProvisionVmJob.
            'ci_user' => config('provisioning.ci_user', 'ubuntu'),
            'ci_password' => Str::password(20, letters: true, numbers: true, symbols: false, spaces: false),
            // Stage 8: inject the portal's Ansible automation public key via cloud-init so hardening can
    ...
    /** The Ansible automation SSH public key (for cloud-init sshkeys); null if not configured. */
    private function ansiblePublicKey(): ?string
    {
        $p = config('provisioning.ansible_public_key_path');

        return ($p && is_file($p)) ? trim((string) file_get_contents($p)) : null;
    }
