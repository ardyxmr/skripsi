...
return Application::configure(basePath: dirname(__DIR__))
    ...
    // Broadcasting auth on the `api` group + /api prefix. With statefulApi() (below) the api group is
    // stateful, so `auth:sanctum` resolves channel subscriptions from the HttpOnly session cookie. The
    // route also inherits the existing api/* CORS rule.
    ->withBroadcasting(
        __DIR__.'/../routes/channels.php',
        ['prefix' => 'api', 'middleware' => ['api', 'auth:sanctum']],
    )
    ->withMiddleware(function (Middleware $middleware): void {
        // Cookie-based SPA auth: prepend EnsureFrontendRequestsAreStateful to the api group so requests
        // from SANCTUM_STATEFUL_DOMAINS authenticate via the session cookie (+ CSRF) instead of a token.
