...
# Cookie-based SPA session auth (ADR-24). Sessions go on the :6380 noeviction instance (dedicated DB) —
# NEVER the :6379 allkeys-lru cache, or an evicted session is a random logout.
SESSION_DRIVER=redis
SESSION_CONNECTION=session         # config/database.php redis 'session' connection → :6380, REDIS_SESSION_DB
REDIS_SESSION_DB=2
SESSION_LIFETIME=60                # idle backstop (the frontend idle timer is authoritative while a tab is open)
SESSION_EXPIRE_ON_CLOSE=true       # clear the session cookie on full browser close
SESSION_SECURE_COOKIE=true         # HTTPS in prod
SESSION_DOMAIN=infraprov.example.com
SANCTUM_STATEFUL_DOMAINS=infraprov.example.com   # the SPA origin(s) treated as stateful (cookie auth)

# Broadcasting (channel auth rides the session cookie — no bearer token)
...
```ini
VITE_API_BASE_URL=/api            # relative → same-origin with the API (cookies/CSRF, no CORS). Absolute https URL also works since nginx serves one domain.
VITE_REVERB_APP_KEY=...           # = REVERB_APP_KEY
VITE_REVERB_HOST=                 # blank → client uses the page origin (nginx /app → wss). Set a host only for a direct connection.
VITE_REVERB_PORT=
VITE_IDLE_TIMEOUT_MIN=60          # idle auto-logout (warn at TIMEOUT - WARN)
VITE_IDLE_WARN_MIN=5
```

Cookie auth means the browser only needs the one web origin — the session cookie + CSRF are same-origin, and
Reverb's `/api/broadcasting/auth` is cookie-gated (no token in the bundle). Same-origin also makes the dev VM /
forwarded-port case work through the Vite proxy without exposing :8000/:8080.
