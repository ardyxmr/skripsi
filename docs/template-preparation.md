...
- **`fedora44-cloud` (vmid 9002)** — Fedora Cloud Base 44, EL/SELinux (`--selinux-relabel`), `--cpu host`, agent, serial+VGA console. Published as the **Fedora 44** catalog. **Forced first-login reset live-verified 2026-07-05**: carries `99-pwauth.cfg` (`ssh_pwauth: true`) + `99-pwexpire.cfg` (`runcmd: chage -d 0 sysuser`); `runcmd` fires on Fedora 44's cloud-init exactly like Rocky, and a fresh clone forces the change on first **`sysuser`** SSH login. **Verify as `sysuser` (password), never the key-based `sysadmin` — key logins never trigger the expiry prompt.**
- **`rhel10-cloud` (vmid 9003)** — RHEL 10 Package-mode Guest image (build via Red Hat image builder in **Package mode**, NOT Image mode/bootc), EL/SELinux (`--selinux-relabel`), `--cpu host`, agent **shipped** (no `--install`), serial+VGA console. Published as the **RHEL 10** catalog. **Forced first-login reset live-verified 2026-07-05** (same `99-pwauth.cfg` + `99-pwexpire.cfg` contract; verify as `sysuser`). **Proxmox gotcha:** if `virt-customize` reports *"no operating systems were found"*, `export LIBGUESTFS_BACKEND=direct` first.
