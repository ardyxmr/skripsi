...
**Consequences.** A hardening failure never destroys the VM — the VM lifecycle remains Active and the failure is surfaced for retry/inspection. Terraform remains the only path that creates or destroys infrastructure; Ansible only configures what already exists. Skipping hardening leaves the VM in its template default state. Providers added later are unaffected: the hardening stage operates on a discovered IP, independent of provider type.

**Amendment (Stage 8 implementation).** Hardening is no longer a provision-time checkbox; it is a **catalog-bound, on-demand, approval-gated lifecycle action** (which embodies "decoupled from the Terraform lifecycle" more fully than a create-time flag). (1) An **admin uploads the OS-specific playbook to a catalog** (single `.yml` or a `.tar.gz`/`.zip` bundle, syntax-checked, stored on the private disk); since a catalog maps 1:1 to a template, the right playbook is unambiguous. (2) A VM shows a **Hardening** action only when its catalog has a playbook; the action routes through `LifecycleService` (User → approval; Manager/Admin → immediate) exactly like the other lifecycle types. (3) Ansible connects **key-based** via an automation public key injected into every VM by cloud-init at provision time — independent of the template's `ssh_pwauth`. (4) Each run gets an **isolated, retained workspace** `storage/app/ansible-workspaces/{inventory_id}/{ts}_{actor}/` (dynamic inventory + `ansible-execution.log` + `run.json`) mirroring the Terraform per-VM workspace (ADR-08); the DB `HARDEN_VM` audit references the log path (ADR-12). The applied version + `last_hardened_at` are tracked per VM. **Versioning follow-up:** a catalog holds **many named/versioned, immutable** playbooks (`catalog_hardening_versions`, retired via `is_active`, never hard-deleted); the user **selects a version** to apply (carried in the approval payload), and re-running the **same applied version** requires an explicit **`force`** confirm (drift remediation) while a different version proceeds normally.
...
**Decision.** Broadcast those events over **Laravel Reverb** (first-party WebSockets). Events `implements ShouldBroadcast` (**queued**, so a down/slow Reverb fails the broadcast in isolation and never breaks the mutation). **Channel authorization mirrors the API read-scope exactly** — WebSockets must never be a wider data path than HTTP: per-owner `user.{id}` (a manager's group-members' VM events are routed to the manager's *own* channel, so no group channels leak), `role.privileged` (all approvals), presence `role.admin` (fleet + "who's online"). Channel auth originally used a bearer-token Echo authorizer; **superseded by ADR-24** — it now rides the Sanctum stateful **session cookie** (the Echo authorizer sends credentials + the CSRF header). The single frontend poller **demotes to a heartbeat when the socket is connected and reverts to full adaptive polling if it drops** — graceful degradation, never an outage. **Redis runs as two instances**: cache (`allkeys-lru`, evictable) vs queue + Reverb pub/sub (`noeviction` + AOF) — a job queue must never silently evict a job; Reverb's multi-node scaling rides the queue instance's pub/sub. **Consistency stays a write-time guarantee** (the ADR-22 transitional guards), not a transport one — push only narrows the stale-read window.
...
## ADR-24 — Cookie-based SPA session auth (HttpOnly) + Redis sessions + idle logout

**Context.** Auth originally issued a Sanctum **bearer token** that the SPA held in browser storage. Any storage that JavaScript can read is exfiltratable by an XSS bug, and the tokens never expired. The SPA and API are first-party and same-site, so a stateful cookie session is both viable and stronger.

**Decision.** Switch to **Sanctum stateful SPA auth**: login establishes a server session and the browser holds only an **HttpOnly, SameSite session cookie** that JS cannot read; **no token lives in the client**. State-changing requests are protected by the **XSRF-TOKEN/`X-XSRF-TOKEN` CSRF** pair (primed via `GET /sanctum/csrf-cookie`); the `api` middleware group is made stateful (`statefulApi()`), so `/api/*` *and* `/api/broadcasting/auth` authenticate from the cookie (this is what supersedes ADR-23's bearer authorizer). **Sessions are stored in Redis on the `noeviction` instance (its own DB), never the `allkeys-lru` cache** — an evicted session is a random logout (parallels the ADR-23 queue/cache split). An **idle auto-logout** (warn at 55 min, log out at 60) is enforced client-side (authoritative, since the live poller otherwise keeps the server session warm) with `SESSION_LIFETIME` as the server backstop. Two auth-surface hardenings ship alongside: **login is rate-limited** (email+IP, 5 fails → 429, failed attempts audited) and **`vm_name` is charset-validated** (DNS-label) at the API *and* re-checked before it touches the Terraform workspace path — closing a path-traversal + `${}`-interpolation vector. In dev (VM/forwarded-port setups) the SPA reaches the backend **same-origin via the Vite proxy** (`/api`, `/sanctum`, `/storage`, `/app`), mirroring the prod nginx single-domain model so only one port is exposed.

**Consequences.** XSS can no longer steal a session (HttpOnly); sessions expire (idle + lifetime). CSRF priming + `withCredentials` are now required on every client call, and any new client must `GET /sanctum/csrf-cookie` before its first write. Cross-origin/cross-site browser access needs the origin in `SANCTUM_STATEFUL_DOMAINS` + CORS `supports_credentials` (or, preferably, same-origin via proxy/nginx). The Postgres `sessions` table is retained so `SESSION_DRIVER=database` is an instant rollback.
...
| 14 | Ansible hardening — catalog-bound, on-demand, approval-gated (amended Stage 8) | Config decoupled from infra lifecycle; OS-correct playbook; audited isolated runs |
...
| 24 | Cookie-based SPA session auth (HttpOnly) + Redis sessions + idle logout | No JS-readable token (XSS-safe); sessions expire; channel auth via cookie |
