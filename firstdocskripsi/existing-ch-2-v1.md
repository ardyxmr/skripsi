...
| 3 | Marzuki dkk. (2023) | Otomatisasi konfigurasi jaringan virtual Open vSwitch | Ansible, Proxmox, Open vSwitch | Mengotomatiskan konfigurasi jaringan virtual | Berfokus pada jaringan, tanpa alur permintaan dan persetujuan mesin virtual |
...
| 6 | Nabila & Indrawati (2025) | Kendala pengelolaan infrastruktur teknologi informasi manual | Kajian | Mengidentifikasi inefisiensi dan risiko konfigurasi manual | Mengidentifikasi masalah, belum menghasilkan solusi aplikasi |
...
menyembunyikan kompleksitas *Infrastructure as Code* (IaC) dan *Command Line Interface* (CLI) dari pengguna non-pakar. Penelitian ini mengisi celah tersebut
...
(Proxmox VE) adalah platform virtualisasi sumber terbuka berbasis kernel Linux dan hypervisor *Kernel-based Virtual Machine* (KVM)
yang menyediakan pengelolaan mesin virtual serta antarmuka berbasis web dan *Application Programming Interface* (API)
...
berlisensi sumber terbuka *Mozilla Public License* (MPL) 2.0 di bawah Linux Foundation
...
tugas melalui playbook dan terhubung ke target melalui *Secure Shell* (SSH). Hariyadi dan Marzuki (2020) serta
...
STRIDE adalah kerangka pemodelan ancaman yang mengelompokkan ancaman menjadi enam kategori, yaitu *Spoofing*, *Tampering*, *Repudiation*,
*Information Disclosure*, *Denial of Service*, dan *Elevation of Privilege* [SITASI DIBUTUHKAN: STRIDE / Shostack]. Penelitian ini memakai STRIDE untuk memetakan
...
Frontend sistem memakai React.js dengan pendekatan *Single Page Application* (SPA), sehingga navigasi antar
...
otomatis; C7 abstraksi self-service tanpa keterampilan IaC/CLI; dan C8 kesesuaian untuk usaha kecil dan menengah (UKM).
...
| Platform *Cloud Management Platform* (CMP) komersial (Morpheus, VMware Aria) | Tidak | Separuh | Penuh | Penuh | Penuh | Separuh | Penuh | Tidak |
...
melalui plugin resmi *foreman_fog_proxmox*; dan dukungan Proxmox pada Morpheus melalui plugin Hewlett Packard Enterprise (HPE) [SITASI DIBUTUHKAN: dokumentasi resmi masing-masing perangkat]. Perangkat yang mengabstraksikan
...
konstruktif. Pertanyaan perancangan dan tata kelola (Rumusan Masalah 1 dan Rumusan Masalah 2) dijawab melalui pembangunan
...
(Rumusan Masalah 3) dan kebergunaan (Rumusan Masalah 4), terhadap antarmuka Proxmox bawaan. Hipotesis dirumuskan secara
direksional dan deskriptif karena jumlah operator uji terbatas, sehingga penelitian tidak mengklaim
signifikansi statistik.

- **H1 (Efisiensi, jumlah aksi).** Jumlah aksi operator pada aplikasi bersifat tetap atau konstan (O(1)) dan
  lebih sedikit dibanding antarmuka Proxmox bawaan yang tumbuh linear (O(N)) seiring bertambahnya
  jumlah mesin virtual yang disimbolkan N.
...
Hipotesis H1 sampai H3 menjawab Rumusan Masalah 3, sedangkan H4 menjawab Rumusan Masalah 4. Bab III menjabarkan rancangan
