    ...
    <tr key={`sk-${r}`} className="animate-pulse border-b border-gray-100 dark:border-theme last:border-0">
      <td colSpan={cols} className="px-5 py-4">
        <div className="flex items-center gap-4">
          <div className="h-4 w-4 rounded bg-gray-100 dark:bg-slate-700/40 shrink-0" />
          {Array.from({ length: bars }).map((_, b) => (
            <div key={b} className={`h-4 rounded bg-gray-100 dark:bg-slate-700/40 ${WIDTHS[b % WIDTHS.length]}`} />
          ))}
        </div>
      </td>
    </tr>
