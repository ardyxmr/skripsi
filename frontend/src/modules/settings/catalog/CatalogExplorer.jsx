...
import { Layers, Globe, Server, Database, Cloud, AlertCircle, Clock, X, Shield } from 'lucide-react';
...
export default function CatalogExplorer({ catalogDrawer, setCatalogDrawer }) {
  ...
  const [hvVersions, setHvVersions] = useState([]);   // Harden/Patch playbook versions (read-only)
  const catalogId = catalogDrawer.catalog?.id;
  useEffect(() => {
    if (!catalogDrawer.isOpen || !catalogId) { setHvVersions([]); return; }
    let active = true;
    api.get(`/catalogs/${catalogId}/hardening`).then((r) => active && setHvVersions(r || [])).catch(() => active && setHvVersions([]));
    return () => { active = false; };
  }, [catalogDrawer.isOpen, catalogId]);
  ...
  return (
    <div className={`fixed inset-y-0 right-0 w-[800px] bg-slate-50 dark:bg-[#18181b] shadow-2xl z-[150] transform transition-transform duration-300 ease-in-out flex flex-col ${catalogDrawer.isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      {catalogDrawer.catalog && (
        <div className="flex flex-col h-full h-screen overflow-hidden">
          ...
          <div className="bg-white dark:bg-card border-b border-slate-200 dark:border-theme p-6 shrink-0 flex items-start justify-between z-10 shadow-sm">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-zinc-100">{catalogDrawer.catalog.name}</h2>
                  <p className="text-[13px] text-slate-500 dark:text-zinc-400 mt-1">{catalogDrawer.catalog.description || 'No description provided'}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="text-slate-300 dark:text-zinc-600">|</span>
                <span className="text-[12px] font-medium text-slate-600 dark:text-zinc-400 flex items-center gap-1.5"><Server size={14} className="text-slate-400" /> {catalogDrawer.catalog.activeVMs || 0} Active VMs</span>
                <span className="text-slate-300 dark:text-zinc-600">|</span>
                <span className="text-[12px] font-medium text-slate-600 dark:text-zinc-400 flex items-center gap-1.5"><Clock size={14} className="text-slate-400" /> Updated {catalogDrawer.catalog.lastUpdated}</span>
              </div>
            </div>
            <button onClick={() => setCatalogDrawer({ isOpen: false, catalog: null })} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300 rounded-md hover:bg-slate-100 dark:hover:bg-zinc-700 transition-colors">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
            ...
            {/* Harden / Patch playbooks (read-only; managed in the catalog edit modal) */}
            <div className="bg-white dark:bg-card border border-slate-200 dark:border-theme rounded-card p-5 shadow-sm">
              <h3 className="text-[13px] font-bold text-slate-800 dark:text-zinc-200 mb-4 flex items-center gap-2"><Shield size={16} className="text-teal-500" /> Harden / Patch Playbooks</h3>
              {hvVersions.length === 0 ? (
                <span className="text-[12px] text-slate-500">No playbooks uploaded — VMs from this catalog won’t show the Harden / Patch action.</span>
              ) : (
                <div className="border border-slate-200 dark:border-theme rounded-md divide-y divide-slate-100 dark:divide-theme overflow-hidden">
                  {hvVersions.map((v) => (
                    <div key={v.id} className="flex items-center gap-3 px-3 py-2">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${v.isActive ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-zinc-700 dark:text-zinc-400'}`}>
            ...
            <div className="bg-white dark:bg-card border border-slate-200 dark:border-theme rounded-card p-5 shadow-sm">
              <h3 className="text-[13px] font-bold text-slate-800 dark:text-zinc-200 mb-4 border-b border-slate-100 dark:border-theme pb-2 flex items-center justify-between">
              ...
              <div className="relative mt-6 mb-4 px-4">
                  ...
                  <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[14px] font-bold text-slate-800 dark:text-zinc-200">{catalogDrawer.catalog.provider}</span>
                ...
                <div className="relative z-10 flex items-start gap-4">
                  ...
                  <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[14px] font-bold text-slate-800 dark:text-zinc-200">{catalogDrawer.catalog.template}</span>
