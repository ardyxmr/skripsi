...
import { 
  X, CheckCircle2, Box, Server, Database, Share2, ServerCrash, Zap, Settings2, Globe, Clock, Shield, AlertTriangle, AlertCircle, XCircle, Cloud, Cpu, HardDrive
} from 'lucide-react';
...
export default function DatastoreDiscovery({ datastoreDrawer, setDatastoreDrawer }) {
  ...
  return createPortal(
    <>
      <div className={`fixed inset-y-0 right-0 w-[800px] bg-slate-50 dark:bg-[#0f172a] shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${datastoreDrawer.isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="bg-white dark:bg-card border-b border-slate-200 dark:border-theme p-6 shrink-0 flex items-start justify-between z-10 shadow-sm relative">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Datastore Explorer</h2>
                  <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Mapping overview for {datastoreDrawer.datastore.name}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 mt-2">
                <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${datastoreDrawer.datastore.status === 'Active' ? 'bg-emerald-50 border border-emerald-200 text-emerald-700 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-400' : datastoreDrawer.datastore.status === 'Offline / Missing' ? 'bg-rose-50 border border-rose-200 text-rose-700 dark:bg-rose-500/10 dark:border-rose-500/20 dark:text-rose-400' : datastoreDrawer.datastore.status === 'Low Capacity' ? 'bg-amber-50 border border-amber-200 text-amber-700 dark:bg-amber-500/10 dark:border-amber-500/20 dark:text-amber-400' : 'bg-slate-50 border border-slate-200 text-slate-700 dark:bg-surface dark:border-theme dark:text-slate-400'}`}>
                ...
                <span className="text-slate-300 dark:text-slate-600">|</span>
                <span className="text-[12px] font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5"><Server size={14} className="text-slate-400" /> {datastoreDrawer.datastore.activeVMs || 0} Active VMs</span>
                <span className="text-slate-300 dark:text-slate-600">|</span>
                <span className="text-[12px] font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5"><Clock size={14} className="text-slate-400" /> Updated {datastoreDrawer.datastore.lastUpdated}</span>
              </div>
            </div>
            <button onClick={() => setDatastoreDrawer({ isOpen: false, datastore: null })} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
              <X size={20} />
            </button>
          </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-white dark:bg-card border border-slate-200 dark:border-theme rounded-card p-5 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {(Array.isArray(datastoreDrawer.datastore.environment) ? datastoreDrawer.datastore.environment : [datastoreDrawer.datastore.environment]).map(env => (
              ...
              <div className="bg-white dark:bg-card border border-slate-200 dark:border-theme rounded-card p-5 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {datastoreDrawer.datastore.tiers?.map(tier => (
              ...
              <div className="relative mt-6 mb-4 px-4">
                    ...
                    <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-[14px] font-bold text-slate-800 dark:text-slate-200">{datastoreDrawer.datastore.provider}</span>
                  ...
                  <div className="relative z-10 flex items-start gap-4 mb-8">
                    ...
                    <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-[14px] font-bold text-slate-800 dark:text-slate-200">{datastoreDrawer.datastore.node || 'pve01'}</span>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                  ...
                  <div className="relative z-10 flex items-start gap-4 mb-8">
                    ...
                    <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-[14px] font-bold text-slate-800 dark:text-slate-200">{datastoreDrawer.datastore.providerDatastore}</span>
                    ...
                    <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col gap-1">
                          <span className="text-[14px] font-bold text-slate-800 dark:text-slate-200">{datastoreDrawer.datastore.name}</span>
