    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-card rounded-modal shadow-modal border border-gray-200 dark:border-theme w-full max-w-[600px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 max-h-[90vh]">
        <div className="shrink-0 flex items-center justify-between p-4 border-b border-gray-100 dark:border-theme">
          <h3 className="font-semibold text-slate-800 dark:text-slate-100">{modal.mode === 'add' ? 'Add Datastore' : 'Edit Datastore'}</h3>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 flex flex-col gap-5">
            <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-theme rounded-md p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Discovered Storage <span className="text-rose-500">*</span></label>
                  <select name="providerDatastoreId" defaultValue={modal.data?.providerDatastoreId || ''} required disabled={!providerId || loading} className={`${inputCls} cursor-pointer disabled:opacity-50`}>
                    <option value="" disabled>{!providerId ? 'Select a provider first' : loading ? 'Loading…' : 'Select discovered storage'}</option>
                    {datastores.map((s) => (
