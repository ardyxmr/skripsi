  ...
  return (
    <div className="flex flex-col gap-6 h-full animate-in slide-in-from-right-8 fade-in duration-300 fill-mode-both items-start w-full">
      ...
      <div className="shrink-0 w-full bg-white dark:bg-card border border-gray-200 dark:border-theme rounded-card p-4 shadow-card flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100">Environment Management Statistics</h1>
          <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Configure expiry, approval, and resource policies per environment.</p>
        </div>
        <div className="flex gap-4">
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-200">{totalEnvs}</div>
            <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-medium">Total Environments</div>
          </div>
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
          ...
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
            ...
            <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-medium">Default Envs</div>
          </div>
          <div className="text-center px-4">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{customEnvs}</div>
            <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-medium">Custom Envs</div>
          </div>
        </div>
      </div>
                    ...
                    <tr key={env.id} className="table-row-optimized border-b border-slate-100 dark:border-theme last:border-0 group">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-slate-800 dark:text-slate-200">{env.name}</p>
                            <p className="text-[12px] text-slate-500 dark:text-slate-500 truncate max-w-[200px]" title={env.description}>{env.description}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        {env.type === 'Default' ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[12px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                      ...
                      <td className="px-5 py-3 text-center">
                        ...
                        >
                          <button 
                            ...
                            className="w-full px-3 py-2 text-left text-[13px] text-blue-600 dark:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2"
                          >
              ...
              {envActionModal.isBlocking ? (
                <>
                  ...
                  <div className="flex justify-end mt-2">
                    <button onClick={() => setEnvActionModal({ isOpen: false, action: null, environment: null, isBlocking: false, blockReasons: [] })} className="px-4 py-2 text-[13px] font-medium bg-slate-100 dark:bg-surface text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md transition-colors">Close</button>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="bg-slate-50 dark:bg-surface p-3 rounded-card text-[12px] border border-gray-200 dark:border-theme">
                    <div className="font-semibold text-slate-700 dark:text-slate-300 mb-1">Environment:</div>
                    <div className="text-slate-500 dark:text-slate-400 font-mono">{envActionModal.environment?.name}</div>
                  </div>
                  <div className="flex flex-col gap-1.5 mt-2">
                    <label className="text-[12px] font-medium text-slate-700 dark:text-slate-300">Type the environment name to confirm:</label>
                    <input 
                      type="text" 
                  ...
                  <div className="p-4 bg-transparent dark:bg-transparent/50 border-t border-gray-100 dark:border-theme flex justify-end gap-3 mt-2 -mx-5 -mb-5">
                    <button onClick={() => setEnvActionModal({ isOpen: false, action: null, environment: null, isBlocking: false, blockReasons: [] })} className="px-4 py-2 text-[13px] font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-input transition-colors">
                      Cancel
                    </button>
                    <button 
