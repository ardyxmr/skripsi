...
export default function NetworkExplorer({ networkDrawer, setNetworkDrawer }) {
  ...
  return createPortal(
    <>
      ...
      <div className={`fixed inset-y-0 right-0 w-[800px] bg-slate-50 dark:bg-[#18181b] shadow-2xl z-[200] transform transition-transform duration-300 ease-in-out flex flex-col ${networkDrawer.isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="bg-white dark:bg-card border-b border-slate-200 dark:border-theme p-6 shrink-0 flex items-start justify-between z-10 shadow-sm relative">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-800 dark:text-zinc-100">Network Explorer</h2>
                    <p className="text-[13px] text-slate-500 dark:text-zinc-400 mt-1">Mapping overview for {networkDrawer.network.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <span className="text-slate-300 dark:text-zinc-600">|</span>
                  <span className="text-[12px] font-medium text-slate-600 dark:text-zinc-400 flex items-center gap-1.5"><Server size={14} className="text-slate-400" /> {networkDrawer.network.activeVMs || 0} Active VMs</span>
                  <span className="text-slate-300 dark:text-zinc-600">|</span>
                  <span className="text-[12px] font-medium text-slate-600 dark:text-zinc-400 flex items-center gap-1.5"><Clock size={14} className="text-slate-400" /> Updated {networkDrawer.network.lastUpdated}</span>
                </div>
              </div>
              <button onClick={() => setNetworkDrawer({ isOpen: false, network: null })} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300 rounded-md hover:bg-slate-100 dark:hover:bg-zinc-700 transition-colors">
                <X size={20} />
              </button>
            </div>
                ...
                <div className="relative mt-6 mb-4 px-4">
                      ...
                      <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="text-[14px] font-bold text-slate-800 dark:text-zinc-200">{networkDrawer.network.provider}</span>
                    ...
                    <div className="relative z-10 flex items-start gap-4 mb-8">
                      ...
                      <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="text-[14px] font-bold text-slate-800 dark:text-zinc-200">{networkDrawer.network.providerNetwork}</span>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            {/* Follows the live effectiveStatus() (like the top chips): an offline provider/node
                                can't report a "Success/Available" network. */}
                            ...
                            <span className={`text-[11px] font-medium px-2 py-0.5 rounded ${offline ? 'text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10' : 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10'}`}>{offline ? net.status : 'Available'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex-1 bg-slate-50 dark:bg-surface border border-slate-200 dark:border-theme rounded-lg p-3">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col gap-1">
                            <span className="text-[14px] font-bold text-slate-800 dark:text-zinc-200">{networkDrawer.network.name}</span>
