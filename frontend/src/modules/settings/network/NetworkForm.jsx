...
export default function NetworkForm({ modal, setModal, handleAddEditNetworkSubmit, providers, onChange }) {
  ...
  // Grey out Save until the full provider → node → bridge chain and name pass native validation.
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-card rounded-modal shadow-modal border border-gray-200 dark:border-theme w-full max-w-[600px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 max-h-[90vh]">
        <div className="shrink-0 flex items-center justify-between p-4 border-b border-gray-100 dark:border-theme">
          <h3 className="font-semibold text-slate-800 dark:text-zinc-100">{modal.mode === 'add' ? 'Add Network' : 'Edit Network'}</h3>
        ...
        <form ref={formRef} onSubmit={handleAddEditNetworkSubmit} onChange={(e) => { onChange?.(e); syncValidity(); }} className="flex flex-col overflow-hidden min-h-0">
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 flex flex-col gap-5">
            <div className="bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-theme rounded-md p-4">
              <label className="text-[11px] font-semibold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-4 block">Network Information</label>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-[12px] font-semibold text-slate-700 dark:text-zinc-300 mb-1">Network Name <span className="text-rose-500">*</span></label>
