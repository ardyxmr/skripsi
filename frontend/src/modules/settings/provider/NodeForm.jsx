...
export default function NodeForm({ isOpen, mode, data, providers, onSubmit, onClose, onChange }) {
  const [providerId, setProviderId] = useState('');
  const [discoveredNodes, setDiscoveredNodes] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (isOpen) setProviderId(data?.providerId ?? '');
  }, [isOpen, data]);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-card rounded-modal shadow-modal border border-gray-200 dark:border-theme w-full max-w-[600px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 max-h-[90vh]">
        <div className="shrink-0 flex items-center justify-between p-4 border-b border-gray-100 dark:border-theme">
          <h3 className="font-semibold text-slate-800 dark:text-slate-100">{mode === 'add' ? 'Add Node' : 'Edit Node'}</h3>
          <button onClick={onClose} className="p-1.5 text-slate-400 hover:text-slate-500 dark:hover:text-slate-300 transition-colors bg-slate-100 hover:bg-slate-200 dark:bg-surface dark:hover:bg-slate-700 rounded-full">
        ...
        <form onSubmit={onSubmit} onChange={onChange} className="flex flex-col overflow-hidden min-h-0">
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 flex flex-col gap-5">
            <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-theme rounded-md p-4">
              <label className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4 block">Node Information</label>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Node Name <span className="text-rose-500">*</span></label>
                  <input type="text" name="nodeName" defaultValue={data?.nodeName ?? data?.name} required placeholder="e.g. Jakarta Zone A" className={inputCls} />
                </div>
                <div className="col-span-2">
                ...
                <div>
                  ...
                  <select name="providerId" value={providerId} onChange={(e) => setProviderId(e.target.value)} required className={`${inputCls} cursor-pointer`}>
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Discovered Node <span className="text-rose-500">*</span></label>
                  <select name="providerNodeId" defaultValue={data?.providerNodeId || ''} required disabled={!providerId || loading} className={`${inputCls} cursor-pointer disabled:opacity-50`}>
                    <option value="" disabled>{!providerId ? 'Select a provider first' : loading ? 'Loading…' : 'Select discovered node'}</option>
                    {discoveredNodes.map((n) => (
                      <option key={n.id} value={n.id}>{n.nodeName}{n.cpuCount ? ` (${n.cpuCount} vCPU)` : ''}</option>
                    ))}
                  </select>
                  <p className="text-[11px] text-slate-400 mt-1">The raw hypervisor node this friendly name abstracts.</p>
                </div>
                <div className="col-span-2">
          ...
          <div className="shrink-0 px-5 py-4 border-t border-gray-100 dark:border-theme flex items-center justify-end gap-3 bg-white dark:bg-card">
            ...
            <button type="submit" className="px-4 py-2 text-[13px] font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-input transition-colors shadow-sm">{mode === 'add' ? 'Save' : 'Update'}</button>
          </div>
        </form>
      </div>
    </div>
