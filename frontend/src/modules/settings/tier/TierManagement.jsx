...
export default function TierManagement() {
  ...
  const tierColumns = [
    { key: 'name', header: 'Tier Name', weight: 2.4, sortable: true, sortAccessor: (t) => (t.name || '').toLowerCase(),
      render: (tier) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <p className="font-medium text-slate-800 dark:text-zinc-200">{tier.name}</p>
              {tier.type === 'Default' && <Shield size={12} className="text-slate-400" title="Default Tier" />}
            </div>
            <p className="text-[12px] text-slate-500 dark:text-zinc-500 truncate max-w-[200px]" title={tier.description}>{tier.description}</p>
          </div>
        </div>
      ) },
    { key: 'cpu', header: 'CPU', weight: 1.1, sortable: true, sortAccessor: (t) => t.cpu,
      render: (tier) => (
        <span className="inline-flex items-center gap-1.5 text-slate-600 dark:text-zinc-300 font-medium">
          <Cpu size={14} className="text-slate-400" />{tier.cpu} vCPU
        </span>
      ) },
    { key: 'ram', header: 'RAM (GB)', weight: 1.2, sortable: true, sortAccessor: (t) => t.ram,
      render: (tier) => (
        <span className="inline-flex items-center gap-1.5 text-slate-600 dark:text-zinc-300 font-medium">
          <MemoryStick size={14} className="text-slate-400" />{tier.ram} GB
        </span>
      ) },
    { key: 'disk', header: 'Disk (GB)', weight: 1.2, sortable: true, sortAccessor: (t) => t.disk,
      render: (tier) => (
        <span className="inline-flex items-center gap-1.5 text-slate-600 dark:text-zinc-300 font-medium">
          <HardDrive size={14} className="text-slate-400" />{tier.disk} GB
        </span>
      ) },
    { key: 'status', header: 'Status', weight: 1.3, sortable: true, sortAccessor: (t) => t.status,
      render: (tier) => (
    ...
    { key: 'createdDate', header: 'Created Date', weight: 1.7, sortable: true, sortAccessor: (t) => t.createdDate || '',
      cellClassName: 'px-4 py-3 text-slate-500 dark:text-zinc-400 text-[13px]',
      render: (tier) => formatDateTime(tier.createdDate) },
    { key: 'action', header: 'Action', weight: 0.7, align: 'center', resizable: false, headerClassName: 'px-5 py-3', cellClassName: 'px-5 py-3',
      render: (tier) => (
        <TableActionMenu isOpen={activeMenuId === tier.id} onToggle={(e) => openMenu(e, tier.id)} dropdownPos={menuPosition}>
          <button
  ...
  return (
    <div className="flex flex-col gap-6 h-full animate-in slide-in-from-right-8 fade-in duration-300 fill-mode-both items-start w-full">
      ...
      <div className="shrink-0 w-full bg-white dark:bg-card border border-gray-200 dark:border-theme rounded-card p-4 shadow-card flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-bold text-slate-800 dark:text-zinc-100">Tier Management Statistics</h1>
          <p className="text-[13px] text-slate-500 dark:text-zinc-400 mt-1">Configure compute resource blueprints for provisioning.</p>
        </div>
        <div className="flex gap-4">
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
            <div className="text-2xl font-bold text-slate-800 dark:text-zinc-200">{totalTiers}</div>
            <div className="text-[11px] text-slate-500 dark:text-zinc-400 uppercase font-medium">Total Tiers</div>
          </div>
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
            <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{activeTiers}</div>
            <div className="text-[11px] text-slate-500 dark:text-zinc-400 uppercase font-medium">Active Tiers</div>
          </div>
          <div className="text-center px-4 border-r border-slate-200 dark:border-theme">
            <div className="text-2xl font-bold text-amber-600 dark:text-amber-500">{inactiveTiers}</div>
            <div className="text-[11px] text-slate-500 dark:text-zinc-400 uppercase font-medium">Inactive Tiers</div>
          </div>
          <div className="text-center px-4">
            <div className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{mostUsedTier}</div>
            <div className="text-[11px] text-slate-500 dark:text-zinc-400 uppercase font-medium">Most Used Tier</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 w-full overflow-y-auto custom-scrollbar flex flex-col gap-6 pr-1 pb-1">
        ...
        <div className="flex flex-col w-full bg-white dark:bg-card border border-gray-200 dark:border-theme rounded-card shadow-card min-h-0">
          ...
          <DataTable
            ...
            emptyState={{
              icon: Layers,
              message: 'No tiers found matching your criteria.',
            }}
          />
        </div>
      </div>
              ...
              {tierActionModal.isBlocking ? (
                <>
                  ...
                  <div className="flex justify-end mt-2">
                    <button onClick={() => setTierActionModal({ isOpen: false, action: null, tier: null, isBlocking: false, blockReasons: [] })} className="px-4 py-2 text-[13px] font-medium bg-slate-100 dark:bg-surface text-slate-700 dark:text-zinc-300 hover:bg-slate-200 dark:hover:bg-zinc-700 rounded-md transition-colors">Close</button>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="bg-slate-50 dark:bg-surface p-3 rounded-card text-[12px] border border-gray-200 dark:border-theme">
                    <div className="font-semibold text-slate-700 dark:text-zinc-300 mb-1">Tier Blueprint:</div>
                    <div className="text-slate-500 dark:text-zinc-400 font-mono">{tierActionModal.tier?.name} ({tierActionModal.tier?.cpu} vCPU, {tierActionModal.tier?.ram}GB RAM)</div>
                  </div>
                  <div className="flex flex-col gap-1.5 mt-2">
                    <label className="text-[12px] font-medium text-slate-700 dark:text-zinc-300">Type the tier name to confirm:</label>
                    <input 
                      type="text" 
                  ...
                  <div className="p-4 bg-transparent dark:bg-transparent/50 border-t border-gray-100 dark:border-theme flex justify-end gap-3 mt-2 -mx-5 -mb-5">
                    <button onClick={() => setTierActionModal({ isOpen: false, action: null, tier: null, isBlocking: false, blockReasons: [] })} className="px-4 py-2 text-[13px] font-medium text-slate-600 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-700 rounded-input transition-colors">
                      Cancel
                    </button>
                    <button 
