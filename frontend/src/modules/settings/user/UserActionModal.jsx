        ...
        <div className="p-5 flex flex-col items-center text-center gap-3">
          ...
          <div className="text-sm text-slate-600 dark:text-zinc-400">
            {isWarning ? (
              type === 'user_manager' ? (
                ...
                <p>This {type} cannot be deleted because it is assigned to <span className="font-bold text-slate-800 dark:text-zinc-200">{assignedUsers.length} user(s)</span>. Please reassign the users first.</p>
              )
            ) : (
              <p>Are you sure you want to delete <span className="font-bold text-slate-800 dark:text-zinc-200">"{data?.name}"</span>? This action cannot be undone.</p>
            )}
          </div>
          ...
          <div className="flex justify-center gap-3 mt-4 w-full">
            {isWarning ? (
              ...
              <>
                <button onClick={onClose} className="flex-1 px-4 py-2 text-[13px] font-medium text-slate-600 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-700 rounded-input transition-colors">Cancel</button>
